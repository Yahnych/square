...
class Bot {
            ...
            case 32:
                ...
                if (!this.isAlive) {
                    ...
                    if (!user.startedBots) {
                        ...
                        logger.good('[SERVER] [AA] Bots Moving!')
                    }
    ...
    getClosestEntity(type, botX, botY, botSize) {
        ...
        for (const entity of Object.values(this.viewportEntities)) {
            ...
            switch (type) {
                case 'biggerPlayer':
                    isConditionMet = !entity.isVirus && !entity.isPellet && entity.size > botSize * 2 && entity.name !== bots.name
                    break
                case 'pellet':
        ...
        switch (reader.readUint8()) {
            case 0:
                if (!user.startedBots) {
                    ...
                    logger.good('[SERVER]  comenzando...')
                    logger.good("[SERVER]  User capatcha/masa v3 tokens:", user.tokens.length);
                }
                break
            case 1:
